...
} = require('../adapters/storage/storageAdapterContract');

const {
  StorageAdapterError
} = require('../adapters/storage/storageAdapterError');

const {
  SupabaseStorageAdapter
} = require('../adapters/storage/supabaseStorageAdapter');
            ...
            return {
              data: {
                signedUrl:
                  ...
                  `${bucket}/${path}?token=test-${expiresIn}`
              },
              error: null
            };
          }
...
test('assertStorageAdapter rechaza adaptadores incompletos', () => {
  assert.throws(
    () => assertStorageAdapter({ uploadObject() {} }),
    /Faltan métodos/
  );
});
...
test('uploadObject carga contenido usando service role', async () => {
  ...
  assert.equal(
    calls[0].url,
    'https://example.supabase.co/storage/v1/object/designs/client/logo.png'
  );
  ...
  assert.deepEqual(result, {
    bucket: 'designs',
    path: 'client/logo.png',
    key: 'designs/client/logo.png',
    etag: '"abc123"',
    data: { Key: 'designs/client/logo.png' }
  });
});
...
test('getObjectMetadata devuelve metadatos normalizados', async () => {
  ...
  const metadata = await adapter.getObjectMetadata({
    bucket: 'designs',
    path: 'render.webp'
  });
...
test('getObjectMetadata devuelve null para objeto inexistente', async () => {
  ...
  assert.equal(
    await adapter.getObjectMetadata({
      bucket: 'designs',
      path: 'missing.webp'
    }),
    null
...
test('deleteObject elimina mediante prefixes sin borrar el bucket', async () => {
  ...
  assert.equal(
    calls[0].url,
    'https://example.supabase.co/storage/v1/object/designs'
  );
