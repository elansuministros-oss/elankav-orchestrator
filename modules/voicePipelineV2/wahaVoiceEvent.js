...
function extractMedia(payload = {}) {
  ...
  const mimeType = normalizeMimeType(
    ...
    (media && typeof media === 'object' ? media.mimetype || media.mimeType : '') || 'audio/ogg'
  );
...
function isVoicePayload(payload = {}) {
  ...
  return extractMedia(payload).mimeType.startsWith('audio/');
}
...
function normalizeWahaVoiceEvent(body = {}, env = process.env) {
  ...
  return {
    ...
    senderRaw,
    chatId,
    phone: senderRaw.split('@')[0].replace(/\D/g, ''),
...
module.exports = { extractMedia, isVoicePayload, normalizeMimeType, normalizeWahaVoiceEvent };
