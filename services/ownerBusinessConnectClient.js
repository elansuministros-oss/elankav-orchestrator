...
async function listAuthorizedPrices(term,env){const response=await requestConnect(`/api/v1/business/vqs/pricing/catalog${paramsFrom({q:String(term||'').trim()||undefined,limit:100})}`,{},env);const data=response?.data&&typeof response.data==='object'?response.data:{};return{...response,data:{...data,matches:Array.isArray(data.items)?data.items:[]}}}
...
async function sendQuotationEmail(projectId,body={},env){return requestConnect(`/api/v1/business/vqs/quotations/${query(projectId)}/send-email`,{method:'POST',body},env)}
...
module.exports={
  OwnerBusinessConnectError,applyPayment,createAndProcessDesign,createCustomer,createDesignRequest,createLogisticsRule,createOwnerCustomer,createOwnerFamily,createOwnerSeller,createPriceAuthorization,createQuotation,createWorkOrder,
  deactivateOwnerCustomer,deactivateOwnerFamily,deactivateOwnerSeller,deleteOwnerSeller,getDesignRequest,getPayment,getQuotation,listAuthorizedPrices,listCustomers,listLogisticsRules,listOwnerCustomers,listOwnerFamily,listOwnerSellers,listPayments,listPriceAuthorizations,listProviders,listQuotations,listWorkOrders,
  executeMarketplaceTool,getElanGoControl,getProspectTimeline,marketplaceCreateDemand,marketplaceCreateInquiry,marketplaceGetSearchMission,marketplaceListDemands,marketplaceListDiscoveries,marketplaceListDiscoveryBuyers,marketplaceListDiscoveryInterests,marketplaceRecordSearchMissionResults,marketplaceRunMatching,marketplaceUpdateDiscoveryInterest,marketplaceUpdateSearchMissionStatus,marketplaceUpsertDiscovery,marketplaceUpsertDiscoveryBuyer,normalizeQuotationSource,recordElanGoHeartbeat,removeQuotationImage,prospectingInternalToken,requestConnect,requestElanGoControl,requestMarketplaceRuntime,resolveCatalogPricing,reviseDesignRequest,revokePriceAuthorization,searchCatalogMaterials,searchCustomers,searchOwnerContacts,searchProspects,searchProviders,sendDesignWhatsApp,sendOwnerWhatsApp,sendQuotationEmail,sendQuotationWhatsApp,setOwnerSellerPlatforms,updateElanGoControl,updateOwnerCustomer,updateOwnerFamily,updateOwnerSeller,updateQuotation,uploadQuotationImage
};
