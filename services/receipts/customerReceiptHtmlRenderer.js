...
function renderCustomerReceiptHtml(document, options = {}) {
  ...
  const company = { ...brand, ...(options.company || {}) };
...
<section class="signature"><div class="signature-box"><div class="small">Firmado digitalmente por</div><div class="sign">${escapeHtml(signature.displayName || signature.name || executive.name || '')}</div><strong>${escapeHtml(signature.name || executive.name || '')}</strong><br><span class="small">${escapeHtml(signature.role || executive.role || 'Ejecutivo')} · ${escapeHtml(company.displayName || company.name || 'ELANKAV')}</span><br><span class="verified">✓ DOCUMENTO FIRMADO DIGITALMENTE</span></div><div class="delivery"><h3>Entrega digital</h3><div class="status"><span class="dot"></span> Generado</div><p class="small">La descarga del PDF se registrará como evidencia técnica de entrega al dispositivo.</p><div class="row"><span>Canal</span><strong>WhatsApp</strong></div><div class="row"><span>Versión</span><strong>${escapeHtml(document.metadata?.documentVersion || '1')}</strong></div></div></section>
</section>
<footer class="footer"><div>${escapeHtml(company.taxId ? `RUC ${company.taxId}` : 'ELANKAV')} · ${escapeHtml(company.website || '')}</div><div>${escapeHtml(company.whatsapp ? `WhatsApp ${company.whatsapp}` : '')}</div></footer>
</main>
</body>
</html>`;
}
