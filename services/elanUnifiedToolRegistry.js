...
const TOOL_DEFINITIONS = Object.freeze([
  ...
  { name:'buscar_cliente', description:'Busca clientes oficiales.', scope:'customer.read', parameters:qParam },
  ...
  { name:'buscar_proveedor', description:'Busca proveedores oficiales.', scope:'provider.read', parameters:qParam },
  ...
  { name:'buscar_vendedor', description:'Busca vendedores oficiales.', ownerOnly:true, parameters:qParam },
  ...
  { name:'buscar_familiar', description:'Busca familiares autorizados en la autoridad canónica de familia.', ownerOnly:true, parameters:qParam },
  ...
  { name:'buscar_contacto', description:'Busca simultáneamente clientes, proveedores, vendedores y familia.', ownerOnly:true, parameters:qParam },
  ...
  { name:'crear_propuesta_diseno', description:'Crea una solicitud en el motor oficial de diseño de CONNECT.', ownerOnly:true, parameters:{type:'object',properties:{customer:{type:'object'},project:{type:'object'},files:{type:'array',items:{type:'object'}},source:{type:'string'}},required:['project'],additionalProperties:false}},
...
async function executeTool({actor={},tool,arguments:args={},env=process.env}={}){
  ...
  switch(name){
    ...
    case'buscar_cliente':return connect.listOwnerCustomers(requiredText(args.query,'query'),env);
    ...
    case'buscar_proveedor':return connect.listOwnerProviders(requiredText(args.query,'query'),env);
    ...
    case'buscar_vendedor':return connect.listOwnerSellers(requiredText(args.query,'query'),env);
    ...
    case'buscar_familiar':return connect.listOwnerFamily(requiredText(args.query,'query'),env);
    ...
    case'buscar_contacto':return connect.searchOwnerContacts(requiredText(args.query,'query'),env);
    ...
    case'crear_propuesta_diseno':return connect.createDesignRequest({customer:args.customer||{},project:requiredObject(args.project,'project'),files:Array.isArray(args.files)?args.files:[],source:args.source||'elan-unified-runtime'},env);
