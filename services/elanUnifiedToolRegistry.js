...
const TOOL_DEFINITIONS = Object.freeze([
  ...
  { name:'enviar_propuesta_diseno', description:'Envía por WhatsApp una propuesta de diseño ya generada. Requiere el código/token de la propuesta y el teléfono destino.', ownerOnly:true, parameters:{type:'object',properties:{requestCode:{type:'string'},accessToken:{type:'string'},phone:{type:'string'},caption:{type:'string'}},required:['requestCode','accessToken','phone'],additionalProperties:false}},
...
async function executeTool({actor={},tool,arguments:args={},env=process.env}={}){
  ...
  switch(name){
    ...
    case'enviar_propuesta_diseno':return connect.sendDesignWhatsApp(requiredText(args.requestCode,'requestCode'),requiredText(args.accessToken,'accessToken'),requiredText(args.phone,'phone'),optionalText(args.caption),env);
