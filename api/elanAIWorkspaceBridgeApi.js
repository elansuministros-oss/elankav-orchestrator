...
const { executeElanAIWorkspaceQuery } = require('../services/elanAIWorkspaceBridgeService');
...
async function handleElanAIWorkspaceBridgeApi({ req, res, sendJson }) {
  const pathname = new URL(req.url, `http://${req.headers.host || 'localhost'}`).pathname;
  if (pathname !== '/api/internal/elan-ai/workspace-query') return false;
  ...
  try {
    ...
    const actorId = String(req.headers['x-elankav-actor'] || body.actor?.id || 'elan-ai').trim();
    const result = await executeElanAIWorkspaceQuery({
      ...body,
      actor: { ...(body.actor || {}), id: actorId, type: body.actor?.type || 'service' }
    });
    sendJson(res, result.success ? 200 : 400, result);
  } catch (error) {
    const status = error.code === 'REQUEST_TOO_LARGE' ? 413 : 400;
    sendJson(res, status, { success: false, error: { code: error.code || 'WORKSPACE_BRIDGE_ERROR', message: error.message } });
  }

  return true;
}

module.exports = { handleElanAIWorkspaceBridgeApi };
